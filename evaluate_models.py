...
def evaluate_one_model(model_dirpath):
    ...
    for scan_battery_dirname in glob_glob(os_path_join(SCAN_BATTERIES_DIRNAME, '*')):
        process_single_scan_battery(new_folder_name, scan_battery_dirname)
...
if __name__ == '__main__':
    ...
    console = logging_StreamHandler()
    if verbose is True:
        console.setLevel(logging_DEBUG)
        # logger.setLevel(logging_DEBUG)
    else:
        console.setLevel(logging_INFO)
    ...
    for model_index, model_folder in enumerate(models):
        ...
        evaluate_one_model(model_folder)
